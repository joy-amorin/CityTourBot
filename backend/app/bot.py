...
def handle_unrecognized_event_type(user_query):
...
def get_online_events_response():
    ...
    if events:
        ...
        return handle_unrecognized_event_type("")

def get_events_this_month_response():
...
def get_events_by_specific_month_response(user_query):
...
def chat(query):
    ...
    if conversation_state["last_question"] == "tipo_evento":
        if user_query in ["eventos presenciales", "presenciales", "presencial", "en persona", "físicos"]:
            ...
            return get_online_events_response()
        else:
            # If the answer is not valid, repeat the question
            return handle_unrecognized_event_type(user_query)
    ...
    if conversation_state["last_question"] == "evento_presencial_mes":
        if user_query in ["mes actual", "actual", "este mes"]:
            return get_events_this_month_response()
        ...
        # Specific month query
            ...
            return get_events_by_specific_month_response(user_query)
        else:
            # If the answer is not valid, repeat the question
            return handle_unrecognized_month(user_query)
